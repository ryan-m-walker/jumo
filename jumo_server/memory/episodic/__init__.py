...
from jumo_server.memory.episodic.db.episodic_memory_queue import (
    episodic_memory_queue_collection,
)
from jumo_server.memory.episodic.db.short_term_collection import (
    short_term_episodic_memory_db,
)
...
class EpisodicMemory:
    ...
    async def push_to_short_term_summary_queue(self, messages: list):
        await episodic_memory_queue_collection.update_one(
            {}, {"$push": {"short_term_summary_queue": {"$each": messages}}}
        )
