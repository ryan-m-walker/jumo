...
memory_processing_queue: AsyncCollection[MemoryProcessingQueue] = db[
...
async def clear_fact_queue():
    await memory_processing_queue.update_one({}, {"$set": {"fact_queue": []}})


async def push_to_short_term_memories(messages: List[Message]):
    await memory_processing_queue.update_one(
        {}, {"$push": {"short_term_queue": {"$each": messages}}}
    )
    ...
    if len(updated["short_term_queue"]) >= SHORT_TERM_QUEUE_SIZE:
        asyncio.create_task(
            memory_manager.process_short_term_queue(updated["short_term_queue"])
        )


async def clear_short_term_memories():
    await memory_processing_queue.update_one({}, {"$set": {"short_term_queue": []}})


async def push_to_higher_order_memories(memories: List[str]):
    await memory_processing_queue.update_one(
        {}, {"$push": {"higher_order_queue": {"$each": memories}}}
    )
